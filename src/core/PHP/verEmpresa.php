...
$con = mysql_connect("websalvaxzjuan.mysql.db", "websalvaxzjuan", "Juanito123"); 
	mysql_select_db("websalvaxzjuan", $con);
		...
		$sql= "SELECT e.id, e.nombre, s.nombre as 'sector',s.id as 'id_sector', e.direccion,p.nombre as 'poligono',p.id as 'id_poligono', e.telefono, e.movil,  e.email, e.web, e.descripcion, e.foto, e.lati, e.longi FROM empresa e INNER JOIN sector s on s.id=e.id_sector INNER JOIN poligono p on p.id=e.id_poligono Where enWeb=1 ORDER BY rand() ";
