<?php
	...
	mysql_select_db("websalvaxzjuan", $con);
		...
		$sql= "SELECT e.id, e.nombre, s.nombre as 'id_sector', e.direccion,p.nombre as 'id_poligono', e.telefono, e.movil, e.fax, e.email, e.web, e.descripcion, e.foto FROM empresa e INNER JOIN sector s on s.id=e.id_sector INNER JOIN poligono p on p.id=e.id_poligono order by e.nombre asc";
